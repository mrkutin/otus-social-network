...
const connect = async () => {
    try {
        ...
        console.error('Feeds connection error, reconnecting in 1000 ms')
...
const updateCache = async feed => {
    ...
    for (const user_id in postsByUserId) {
        //clean up old post ids from the monolith's feed
        ...
        //build up new cache for the monolith
        for (const post of postsByUserId[user_id]) {
            //add post id to monolith feed list
