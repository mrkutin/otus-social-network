...
//get
router.get('/', (req, res) => {
    return res.send('Hello from dialog service')
})

router.post('/dialog/:user_id/send', authenticate, async (req, res) => {
    console.log(`Dialog service, /dialog/:user_id/send endpoint req.headers["x-request-id"]: ${req.headers["x-request-id"]}`)
    if (!req.user) {
...
router.get('/dialog/:user_id/list', authenticate, async (req, res) => {
    console.log(`Dialog service, /dialog/:user_id/list endpoint req.headers["x-request-id"]: ${req.headers["x-request-id"]}`)
    if (!req.user) {
        return res.status(401).send('Пользователь не аутентифицирован')
    }
